...
- Answer anything outside the eight with the limits sentence: "${LIMITS_SENTENCE}" That covers libraries outside the eight, frameworks other than React and Vue, screen reader behavior, server rendering, and anything the write-up's "What this does not tell you" section lists. Never answer such a question as if it had been measured. Never put a figure in a sentence that names a library, framework or topic outside the eight. If you quote a measured library's figures in that answer at all, give them their own sentence that names only measured libraries.
