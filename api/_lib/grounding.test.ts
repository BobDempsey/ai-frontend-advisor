...
describe('loadGrounding and buildSystemPrompt, on the real repo', () => {
  ...
  it('states the advisor rules from advisor spec section 6', () => {
    ...
    expect(prompt).toContain('When a question names a library, framework or topic outside the eight, answer with the limits sentence and quote no figures at all in that reply, not even for a measured library named in the same question.');
    expect(prompt).toContain('The reader can ask about a measured library separately.');
  });
